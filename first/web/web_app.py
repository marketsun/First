...
def crawl_news():
    """뉴스 크롤링 (Google News RSS + Naver Selenium) - app.py와 동일"""
    try:
        data = request.json
        ...
        # ================================================================
        # 1. Google News (RSS 파싱) - app.py와 동일
        # ================================================================
        try:
            log(f"\n[Google] RSS 파싱 시작...")
        ...
        # ================================================================
        # 2. Naver News (Selenium) - app.py와 동일
        # ================================================================
        try:
            ...
            try:
                ...
                for page in range(1, 4):
                    try:
                        search_url = f"https://search.naver.com/search.naver?where=news&query={quote(keyword)}&start={(page-1)*10+1}"
        ...
        google_news = [{'id': row[0], 'title': row[1], 'link': row[2], 'source': row[3], 'published_time': row[4]} for row in rows]
        
        log(f"\n[완료] 총 {len(google_news)}개 뉴스")
        log(f"{'='*80}\n")
        
        return jsonify({'success': True, 'google_news': google_news, 'naver_news': [], 'logs': logs})
